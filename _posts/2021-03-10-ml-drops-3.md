...
Dependendo da necessidade, é possível configurar o *Data Capture* para registrar o log de um percentual de todas as chamadas ao Sagemaker Endpoint, salvar apenas os dados da requisição ou da resposta do modelo. Ainda é possível manter no log informações customizadas pelo cliente, bastando que as chamadas ao Sagemaker Endpoint pelo método `invoke_endpoint` possuam o parâmetro `CustomAttributes`.

## Pensamentos finais

O Sagemaker Endpoint se destaca como alternativa de deploy de Machine Learning principalmente por conta de seu ecossistema de serviços e APIs, o *Data Capture* é um exemplo vivo deste fato.

Ao associar um baixo custo de armazenamento desses log através do S3 e gerenciar completamente a captura desses logs, a AWS fornece peça essencial para a evolução contínua de seu modelo.
